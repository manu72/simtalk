...
// Output (target) languages accepted by OpenAI gpt-realtime-translate's
// `session.audio.output.*` configuration. Sourced from the upstream 400
// error returned for unsupported codes; keep in sync with that list.
// Codes are matched against the base-language portion of `Language.bcp47`
// (e.g. `zh-Hans` -> `zh`).
const REALTIME_TRANSLATE_TARGET_CODES: ReadonlySet<string> = new Set([
  "af",
  "ar",
  "az",
  "be",
...
const baseLanguageCode = (bcp47: string): string => bcp47.split("-")[0]!.toLowerCase();

export const isLanguageSupportedAsTarget = (lang: Language): boolean =>
  REALTIME_TRANSLATE_TARGET_CODES.has(baseLanguageCode(lang.bcp47));

export const TARGET_LANGUAGES: readonly Language[] = LANGUAGES.filter(isLanguageSupportedAsTarget);
